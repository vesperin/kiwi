 ...
 */
trait Interpreter extends Configuration with
  VesperLibraryConversions {

  implicit def executionContext: ExecutionContextExecutor =
    ExecutionContext.Implicits.global

  val parser: Parser    = QueryParser()
  ...
  private def getId(id: String): String = id match {
    case null => "none"
    case _    => id
  }
  ...
  private def evalPersist(who:Auth, persist: Persist): Future[Option[Result]] = {
    def makeVesperUrl(id:String): Future[String] = Future("""http://www.vesperin.com/kiwi/render?q=id:""" + id)
    ...
    def produceResult(theCode: Code, tinyUrl: String, id: String): Future[Option[Result]] = {
      Future(Some(Result(info = Some(Info(List(
