...
/**
 * Message object for a request to evaluate a command.
 */
case class Get(who:Auth, command:Command)
...
trait Interpreter {
  ...
  def eval(who:Auth, command: Command): Option[ChangeSummary] = {
    ...
    answer(0) match {
      case inspect:Inspect          => evalInspect(inspect)
      case remove:Remove            => evalRemove(remove)
      case rename:Rename            => evalRename(rename)
      case optimize:Optimize        => evalOptimize(optimize)
      case format:Format            => evalFormat(format)
      case deduplicate:Deduplicate  => evalDeduplicate(deduplicate)
      case _=> None
    }
  }
...
class InterpreterActor extends Actor with Interpreter {
  override def receive = {
    case Get(who, command)   => eval(who, command)
    case _=> None
  }
}
