...
class PostKiwiInspectApiSpec extends Specification with Specs2RouteTest with Kiwi {
  ...
  "Kiwi" should {
    "Return an inspect request for POST requests to the root path" in {
      Post("/kiwi/eval?auth_token=legolas", Command(inspect = Some(Inspect(Code(id = "new", name = "Bootstrap.java", description = "Resource Injector", content = "class Bootstrap {void inject(Object object){}}"))))) ~>
    ...
    "Return an inspect request containing a list of imports for POST requests to the root path" in {
      Post("/kiwi/eval?auth_token=legolas", Command(inspect = Some(Inspect(source = Code(id = "new", name = "Bootstrap.java", description = "Resource Injector", content = "class Bootstrap {void inject(List<Object> object){}}"), imports = true)))) ~>
        sealRoute(routes) ~> check {
